...
SELECT id,first_name, last_name from person WHERE id IN (SELECT id_customer from orders WHERE id IN (SELECT id_order FROM orders_products WHERE id_product IN (SELECT id FROM product WHERE name='Tofu')))
